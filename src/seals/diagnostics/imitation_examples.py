...
def make_random_trans_mat(
    ...
    for start_state in range(n_states):
        for action in range(n_actions):
            ...
            next_states = rand_state.choice(n_states, size=(successors,), replace=False)
...
class RandomMDP(TabularModelPOMDP):
    ...
    def __init__(
        ...
        *,
        ...
        observation_matrix = make_obs_mat(
            n_states=n_states,
            is_random=random_obs,
            obs_dim=obs_dim,
            rand_state=rand_gen,
        )
        transition_matrix = make_random_trans_mat(
            n_states=n_states,
            n_actions=n_actions,
            max_branch_factor=branch_factor,
            rand_state=rand_gen,
        )
        initial_state_dist = make_random_state_dist(
...
class CliffWorld(TabularModelPOMDP):
