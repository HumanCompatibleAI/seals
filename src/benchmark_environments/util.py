...
class AbsorbAfterDoneWrapper(gym.Wrapper):
    ...
    def step(self, action):
        ...
        absorb state. In this artificial absorb state, we stop calling
        `self.env.step(action)` (i.e. the `action` argument is entirely ignored) and
        we return fixed values for obs, rew, done, and info.
        """
        if not self.at_absorb_state:
            ...
            info = {}  # XXX: Also allow custom info for absorb state.
...
def _gym_register_as_decorator(
    ...
    **register_kwargs,
    ...
    (`reward_threshold`, `nondeterministic`, `max_episode_steps`, and `kwargs` are
    parameters with default values in `gym.register` and
